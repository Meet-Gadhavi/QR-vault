...
export const VaultModals: React.FC<VaultModalsProps> = ({
    ...
    const createEditModalRef = useRef<any>(null);
    const limitModalRef = useRef<any>(null);
    const accessModalRef = useRef<any>(null);
    const qrModalRef = useRef<any>(null);

    useEffect(() => {
        if (createEditModalRef.current) createEditModalRef.current.open = isModalOpen;
    }, [isModalOpen]);

    useEffect(() => {
        if (limitModalRef.current) limitModalRef.current.open = isLimitModalOpen;
    }, [isLimitModalOpen]);

    useEffect(() => {
        if (accessModalRef.current) accessModalRef.current.open = isAccessModalOpen;
    }, [isAccessModalOpen]);

    useEffect(() => {
        if (qrModalRef.current) qrModalRef.current.open = !!viewQrVault;
    }, [viewQrVault]);

    useEffect(() => {
        const createEl = createEditModalRef.current;
        if (!createEl) return;
        const handleClose = () => setIsModalOpen(false);
        createEl.addEventListener('close', handleClose);
        return () => createEl.removeEventListener('close', handleClose);
    }, [setIsModalOpen]);

    useEffect(() => {
        const limitEl = limitModalRef.current;
        if (!limitEl) return;
        const handleClose = () => setIsLimitModalOpen(false);
        limitEl.addEventListener('close', handleClose);
        return () => limitEl.removeEventListener('close', handleClose);
    }, [setIsLimitModalOpen]);

    useEffect(() => {
        const accessEl = accessModalRef.current;
        if (!accessEl) return;
        const handleClose = () => setIsAccessModalOpen(false);
        accessEl.addEventListener('close', handleClose);
        return () => accessEl.removeEventListener('close', handleClose);
    }, [setIsAccessModalOpen]);

    useEffect(() => {
        const qrEl = qrModalRef.current;
        if (!qrEl) return;
        const handleClose = () => setViewQrVault(null);
        qrEl.addEventListener('close', handleClose);
        return () => qrEl.removeEventListener('close', handleClose);
    }, [setViewQrVault]);

    return (
        <>
            {/* Create / Edit Vault Modal */}
            <s-modal ref={createEditModalRef} open={isModalOpen ? true : undefined} heading={modalMode === 'CREATE' ? 'Create New Vault' : 'Edit Vault'} accessibilityLabel="Create or Edit Vault dialog">
                <div className="p-6 space-y-6 text-gray-900 dark:text-white max-w-2xl w-full">
                    ...
                    <div className="flex bg-gray-100/80 dark:bg-[#0a0a0a] rounded-2xl border border-gray-200/50 dark:border-white/5 p-1.5 relative shadow-inner">
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-xs sm:text-sm font-bold transition-colors duration-300 z-10 ${vaultType === VaultType.SENDING ? 'text-primary-600 dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                        >
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-xs sm:text-sm font-bold transition-colors duration-300 z-10 ${vaultType === VaultType.RECEIVING ? 'text-primary-600 dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                        >
                        ...
                        <div className="flex gap-3">
                            ...
                            <p className="text-xs text-primary-900 dark:text-primary-300 leading-relaxed font-medium">
                                {vaultType === VaultType.SENDING 
                                    ...
                                    : "Collection Mode (Dropbox-style drop vaults) allows visitors to upload files directly into your vault. Perfect for assignments, client documents, or mass photo collection."}
                            </p>
                        </div>
                    </div>
                    ...
                    {vaultType === VaultType.SENDING ? (
                        <>
                            ...
                            <div>
                                ...
                                <div
                                    className={`border-2 border-dashed rounded-xl p-6 sm:p-8 text-center transition-all relative group cursor-pointer ${isDragging ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/30 scale-[1.02]' : 'border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800'}`}
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/10 flex justify-end gap-3 shrink-0">
                        <s-button onClick={() => setIsModalOpen(false)}>Cancel</s-button>
                        <s-button
            ...
            <s-modal ref={limitModalRef} open={isLimitModalOpen ? true : undefined} heading="Vault Limit Reached" accessibilityLabel="Vault limit reached notice">
                <div className="p-6 text-center text-gray-900 dark:text-white max-w-sm w-full">
                    ...
                    <div className="flex flex-col gap-3">
                        <Link to="/pricing" className="w-full" onClick={() => setIsLimitModalOpen(false)}>
                            <s-button variant="primary" className="w-full bg-primary-600 text-white font-medium">
                                Go to Plans
                            </s-button>
                        </Link>
                        <s-button onClick={() => setIsLimitModalOpen(false)} className="w-full font-medium">
                            Maybe Later
                        </s-button>
                    </div>
                </div>
            ...
            <s-modal ref={accessModalRef} open={isAccessModalOpen ? true : undefined} heading="Access Requests" accessibilityLabel="Access Requests management dialog">
            ...
            {/* QR Modal */}
            <s-modal ref={qrModalRef} open={!!viewQrVault ? true : undefined} heading={viewQrVault?.name || 'QR Code'} accessibilityLabel="QR Code display dialog">
                {viewQrVault && (
                    <div className="p-6 text-center text-gray-900 dark:text-white max-w-sm w-full">
                        ...
                        <div className="space-y-3">
                            <s-button
                                ...
                                variant="primary"
                                className="w-full bg-gray-900 text-white font-bold"
                            >
                                Download QR
                            </s-button>
                            ...
                            <div className="flex gap-2">
                                <a
                                    ...
                                    className="flex-1"
                                >
                                    <s-button variant="primary" className="w-full bg-primary-600 text-white font-bold">Open Link</s-button>
                                </a>
                                <s-button
